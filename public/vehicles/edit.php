...
$statement = $pdo->prepare('SELECT id, make, model, year, vin, price, status FROM vehicles WHERE id = :id');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($errors === []) {
        $statement = $pdo->prepare(
            'UPDATE vehicles SET make = :make, model = :model, year = :year, vin = :vin, price = :price, status = :status WHERE id = :id'
        );
        $statement->execute([
            ':make' => $values['make'],
            ':model' => $values['model'],
